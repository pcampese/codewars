...
def pick_peaks(arr):
	...
	if (len(arr) >= 3):
		...
		for possible_peak_index in xrange(1,len(arr)-1):
			...
			# Test if it's a peak or start of a plateu
			if (possible_peak_value > lookback):
				# Can either be a peak or the start of a plateu
				if (possible_peak_value > lookahead):
					# It's a peak
				...
				if (possible_peak_value == lookahead):
					# We have a small number followed by 2 of the same number,
					# so this could be the start of a plateu
					print('|-- Possible plateu STARTING...')
					plateu_index = possible_peak_index
					plateu_value = possible_peak_value

			# Test for plateu ending
			if (possible_peak_value == plateu_value and
